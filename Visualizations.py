...
#######################################################################################
df = pd.read_sql_query('select q.settled_at, q.created_at, q.id, c.category_id from questions q, question_category_links c where q.id = c.question_id and q.settled_value is not null and q.type = \'binary\'', conn)
...
#monthly.groupby('category_id', index = False)
...
monthly_1.plot(kind='bar', color = 'green')
plt.title('Questions resolved per month', fontsize = 15)
plt.ylabel('Frequency')
plt.show()
####################binary vs multi########################
query = "select type, count(type) from questions group by type"
unset = pd.read_sql_query(query, conn)
query = "select type, count(type) from questions where settled_value is not null group by type"
binq = pd.read_sql_query(query, conn)
binq.index = binq.type
unset.index = unset.type
df = pd.concat([binq, unset]).reset_index(drop = True)
df.index = df.type
df2 = df.groupby('type')
df2['count'].plot(kind = 'bar', stacked= True)

df

bin

binq.plot(kind = 'bar', color = 'green', fontsize = 12)
for i, v in enumerate(binq['count']):
    plt.text(i-0.009,v , str(v), fontweight='bold')

unset.plot(kind = 'bar',fontsize = 12)
for i, v in enumerate(unset['count']):
    plt.text(i-0.009,v , str(v), fontweight='bold')

ind = np.arange(len(binq))
p1 = plt.bar(ind,unset.count)#unset.plot(kind = 'bar')
p2 = plt.bar(ind,binq.count)#binq.plot(kind = 'bar')
...
###Plot 2 : As grid
fig, t= plt.subplots(7, 2,figsize = (8, 10), sharex = True, sharey =True)

fig.text(0.5, -0.005, 'Time of resolution', ha='center', fontsize = 15)
fig.text(-0.005, 0.5, 'Mean absolute error', va='center', rotation='vertical', fontsize = 15)
a = 0
b = 0
for c,num in zip(movingaverageDF.category1.unique(), range(1,15)):
    df0= movingaverageDF[movingaverageDF.category1 == c]
    ax = t[a, b]
    ax.plot(df0['Date'], df0['avgError'], color = 'orange')
    ax.set_title(c) 
...
#######################################################################
userdata = pd.read_csv("D:\\Fall 2019\\CAP\\weighted avg\\usersBigdataNew.csv")
vallist = []
for catg in userdata.category1.unique()[:-1]: 
    vallist.append(userdata[userdata.category1 == catg]['user_id'].nunique() + userdata[userdata.category2 == catg]['user_id'].nunique() )

bincat = pd.DataFrame(vallist, index = userdata.category1.unique()[:-1], columns= ['QuestionsCount'])  
bincat = bincat.sort_values('QuestionsCount')
bincat.plot(kind = 'barh') 
plt.title('Number of Users per category',fontsize=15)
plt.show()   

#######################################################################
### Plot of user weights and error
#######################################################################
subset = userdata[userdata.userweights.between(0.1,0.9)]
len(userdata)
import random
random.seed(42)
usersample = subset.sample(n = 200).reset_index(drop= True)
usersample = usersample.sort_values('error').reset_index(drop= True)

fig, ax = plt.subplots()   
usersample['userweights'].plot(kind = 'line')   
usersample['error'].plot(kind = 'line') 
